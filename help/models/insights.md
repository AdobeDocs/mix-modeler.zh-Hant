...
>title="效率測量值"
>abstract="演算法歸因模型產生的效率測量值表示某個接觸點，在不考慮接觸點數量的情況下，對轉換的相對重要性。 這是以 1 到 5 的等級進行測量。 請注意，接觸點數量愈多並不能保證效率測量值愈高。"


>[!CONTEXTUALHELP]
>id="models_diagnostics_totalvolume"
>title="總數量"
>abstract="總數量是使用者接觸某個接觸點的累計次數，包括出現在達成轉換的路徑上以及未產生轉換的路徑上的接觸點。"
